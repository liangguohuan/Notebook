...
这四种模式中都不会改变`variable`的值，其中，只有在`pattern`中使用了`*`匹配符号时，`%`和`%%`，`#`和`##`才有区别。结构中的`pattern`支持通配符，`*`表示零个或多个任意字符，`?`表示零个或一个任意字符，`[...]`表示匹配中括号里面的字符，`[!...]`表示不匹配中括号里面的字符

```shell
variable=111bbbaaa	 # original data
```

##### 5.3.1 第一种模式 ${variable%pattern}

从`variable`右边开始查找`pattern`替换

```shell
echo ${variable%a} 	 # output: 111bbbaa
```

##### 5.3.2 第二种模式 ${variable%%pattern}

从`variable`右边开始查找`pattern`替换，但是替换模式为**贪婪模式**

```shell
echo ${variable%%a*} # output: 111bbb
```

##### 5.3.3 第三种模式 ${variable#pattern} 

从`variable`左边开始查找`pattern`替换

```shell
echo ${variable#1} 	 # output: 11bbbaaa
```

##### 5.3.4 第四种模式 ${variable##pattern}

从`variable`左边开始查找`pattern`替换，但是替换模式为**贪婪模式**

```shell
echo ${variable##1*} # output: bbbaaa
```
