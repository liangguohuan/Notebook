...
g:unite_no_default_keymappings		*g:unite_no_default_keymappings*
		...
		mappings.  Not recommended.  You shouldn't set this to 1 unless
...
<
		The default value is "find" and the args.
...
g:unite_update_time				*g:unite_update_time*
		The time interval for updating the candidate list as the filter
		text it typed in Msec.
...
g:unite_winheight				*g:unite_winheight*
		...
		Note: This variable is deprecated.  Please use
		|unite#custom#profile()| and |unite-options-winheight| instead.
...
g:unite_cursor_line_highlight		*g:unite_cursor_line_highlight*
		...
		|unite-options-cursor-line-highlight| instead.
...
<Plug>(unite_quick_match_choose_action)
		The same as |<Plug>(unite_quick_match_choose_action)|.
...
<Plug>(unite_narrowing_path)		*i_<Plug>(unite_narrowing_path)*
		The same as |<Plug>(unite_narrowing_path)|.
...
unite#get_candidates({sources}, [, {context}])	*unite#get_candidates()*
		...
		The arguments are the same as with |unite#start()|.
...
the context dictionary.
		...
		-cursor-line-highlight={highlight-name}
		Change cursor line highlight.
		Default: "CursorLine"
...
file_rec/async	Similar to |unite-source-file_rec|, but get files asynchronously.
		...
		Note: If you edit one of the files in Vim, the
		file_rec/async source will update its cache.
		Note: If there are more than
		|g:unite_source_file_rec_max_cache_files| candidates,
...
line		Nominates current buffer lines as candidates.
		...
		"forward"  : Search from the current to the buffer top
		"backward" : Search from the current to the buffer bottom
...
sorter_ftime	Compare |getftime()| sorter.
...
<
						*unite-filter-sorter_selecta
sorter_selecta	Another matched rank order sorter. Uses the scoring algorithm
		from selecta: https://github.com/garybernhardt/selecta.
		If the matched length is shorter, the rank is higher.
		This sorter is useful for file candidate source.
		Note: This sorter requires |if_ruby| support.
>
		call unite#custom#source('buffer,file,file_rec',
...
gather_candidates	Function	(Required*)
			...
			source is registered as |unite-source-is_volatile|,
			gather_candidates() is called on _every_ change of the
			input string. This function takes {args} and {context} as
			parameters and returns a {candidate} list. {args} is a
			list of parameters passed to source via the |:Unite|
			command. {context} is the context information when the
			source is called.
			...
			Unite caches the results of gather_candidates() until the
			unite buffer is closed and is discarded (unless you use
			|:UniteResume|). To cache persistently regardless of
			the life of the unite buffer, a source may implement
			logic to save the cache to disk.
...
is_listed		Number		(Optional)
			...
			Defaults to 1.
					...
					*unite-source-attribute-is_grouped*
is_grouped		Number		(Optional)
			Whether the source candidates are grouped.
			Defaults to 0.
			Grouped candidates must have "group" attribute; see
			|unite-notation-{candidate}|
...
description		String		(Optional)
			...
			Defaults to "".
			Appears in the list shown by the |unite-source-source|:
			    |:Unite| source
...
{candidate}					*unite-notation-{candidate}*
			...
			group			(String)	(Optional)
				The candidate group name.
				It is inserted in candidates automatically.
				Note: Required and used when
				|unite-source-attribute-is_grouped| is non
				zero.
...
default_action		String		(Required)
			...
			is omitted, an error occurs.  However, this hack is used
			only in parent actions.
...
A: You can use the setting below; however, this setting is dangerous.  You may delete
files by mistake.
>
...
A: You can use |unite-options-wipe| option.  But it is used, you cannot use
...
Also, you must clear previous cache in file_rec sources and wait until the cache
is completed.
